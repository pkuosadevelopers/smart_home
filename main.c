...
//a sample of upnp light control poiont
void pthread_upnp_light_control();
...
int main(int argc,char **argv)
{
	...
	*/
	...
	pthread_t pth_upnp_light_control;
	if(pthread_create(&pth_upnp_light_control, NULL, (void*)pthread_upnp_light_control, NULL) != 0)
	{
		DBG("\ninit upnp light failed\n");
		return -1;
	}
...
void pthread_server(int* p_listen_fd)
{
	...
	while(1)
	{
		...
		pthread_mutex_lock(&mutex_head);	//lock the gloable variable head
		client_insert(head, new_client);
		pthread_mutex_unlock(&mutex_head);	//unlock the gloable variable head
...
void pthread_receive_from_client(CLIENT_NODE *p)
{
	...
	while(1)
	{
		...
		else	//that is ret > 0
		{
			if(FD_ISSET(socket, &rdfds))		//detect whether socket can be read
			{
				...
				else if(is_req_msg(buff, 8))
				{
					//defination and initilize the new message node by the connnected client socket
					...
					new_message_node->data = (char*)malloc(sizeof(char) * 9);
					strncpy(new_message_node->data, buff, 8);		//copy 8 bytes, but it's not null-terminated
					new_message_node->data[8] = '\0';				//so, make the one after last char to be '\0'
					new_message_node->size = 8;						//set the size to be 8
					new_message_node->next = NULL;					//the next node to be NULL
					...
					//send to tty	
					int count = tty_send(_tty_fd, buff, 8);
	...
	pthread_mutex_lock(&mutex_head_message);	//lock the gloable variable head_message
	message_delete_all_by_sockfd(head_message, socket);
	pthread_mutex_unlock(&mutex_head_message);	//unlock the gloable variable head_message
	DBG("\n\n%d message are all be deleted\n\n", socket);

	pthread_mutex_lock(&mutex_head);	//lock the gloable variable head
	client_delete(head, socket);		//delete the client from the client nodes list
	pthread_mutex_unlock(&mutex_head);	//unlock the gloable variable head
	DBG("\n\n%d client is deleted from the client nodes list\n\n", socket);
...
void pthread_receive_from_tty()
{
	...
	char *ack = (char*)malloc(sizeof(char) * 10);
	char *start;
	...
	while(1)
	{
		...
		printf("first time: %s, the size is %ld\n", buff, pbuf - buff);

		find_ack(ack, buff, pbuf, 8);
		printf("first time 8 bytes: %s\n", ack);
		
		if(is_ack_msg(ack, 8))	//if this is an ACK message, read another message 
		{
			send_sockfd = find_client_to_send(head_message, ack, strlen(ack));
			pbuf = buff;		//reset the pbuf point to the beginning of buff
			while(1)
			{
				read(fd, pbuf, 1);
			...
			pthread_mutex_lock(&mutex_head_message);	//lock the gloable variable head_message
			message_delete(head_message, ack, send_sockfd);
			pthread_mutex_unlock(&mutex_head_message);	//unlock the gloable variable head_message
		} //if is_akc_msg
	} //while 1
} //pthread_receive_from_tty

void pthread_upnp_light_control()
{
	while(1)
	{
		upnp_light_main(2, "toggle");		//if the light is on, then close it, otherwise open it.
